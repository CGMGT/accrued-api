...
import gt.com.tigo.accruedautomation.dto.ProcessValidationDto;
...
import gt.com.tigo.accruedautomation.service.AccruedIntegratedService;
...
@CrossOrigin
@RestController
@RequestMapping("/accruedintegrated")
public class AccruedIntegratedController extends AbstractController<AccruedIntegratedEntity, AccruedIntegratedService> {
    ...
    @Override
    public ResponseEntity<TigoResponseDto> create(AccruedIntegratedEntity entity, Long requesterId) {
        return null;
    }

    @Override
    public ResponseEntity<TigoResponseDto> update(@RequestBody AccruedIntegratedEntity entity, @PathVariable Long requesterId) {
        LOGGER.debug(String.format("@%s::update(AccruedIntegratedEntity, %d)", this.getClass().getName(), requesterId));
    ...
    public ResponseEntity procesar(@RequestBody ProcessValidationDto processValidationDto) {
        LOGGER.debug(String.format("@%s::procesar(%s)", this.getClass().getName(), processValidationDto));

        try {
            return ResponseEntity.ok(new TigoResponseDto(this.service.procesar(processValidationDto)));
        } catch (Exception ex) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new TigoResponseDto(HttpStatus.INTERNAL_SERVER_ERROR.value(), ex.getMessage()));
        }
    }
